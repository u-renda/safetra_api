...
class Company extends REST_Controller {
	...
	function create_post()
	{
		...
		$pic_name = filter(trim($this->post('pic_name')));
		$phone_number = filter(trim($this->post('phone_number')));
		$logo = filter($this->post('logo'));
	...
	function update_post()
	{
		...
		if ($validation == 'ok')
		{
			$query = $this->the_model->info(array('id_company' => $id_company));
			...
			if ($query->num_rows() > 0)
			{
				...
				{
					$param['updated_date'] = date('Y-m-d H:i:s');
					$update = $this->the_model->update($id_company, $param);
