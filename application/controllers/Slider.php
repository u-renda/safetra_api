...
class Slider extends REST_Controller {
    ...
    {
        parent::__construct();
		$this->load->model('slider_model', 'the_model');
    }
	...
	function create_post()
	{
		...
		$slider_url = filter(trim($this->post('slider_url')));
		
		$data = array();
		if ($slider_url == FALSE)
		{
			$data['slider_url'] = 'required';
			$validation = 'error';
			$code = 400;
		}
	...
	function delete_post()
	{
        ...
        $id = filter($this->post('id_slider'));
        
		$data = array();
        if ($id == FALSE)
		{
			$data['id_slider'] = 'required';
			$validation = "error";
			$code = 400;
		}
        ...
        if ($validation == "ok")
		{
            $query = $this->the_model->info(array('id_slider' => $id));
	...
	function info_get()
	{
		...
		$id_slider = filter($this->get('id_slider'));
	...
	function lists_get()
	{
		...
		if (in_array($order, $this->config->item('default_slider_order')) && ($order == TRUE))
		{
			$order = $order;
		}
		else
	...
	function update_post()
	{
		...
		$id_slider = filter($this->post('id_slider'));
		$slider_url = filter(trim($this->post('slider_url')));
		...
		if ($validation == 'ok')
		{
			$query = $this->the_model->info(array('id_slider' => $id_slider));
			...
			if ($query->num_rows() > 0)
			{
				...
				{
					$param['updated_date'] = date('Y-m-d H:i:s');
					$update = $this->the_model->update($id_slider, $param);
