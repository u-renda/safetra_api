...
class Media extends REST_Controller {
    ...
    {
        parent::__construct();
		$this->load->model('media_model', 'the_model');
    }
	...
	function create_post()
	{
		...
		$media_url = filter($this->post('media_url'));
	...
	function delete_post()
	{
        ...
        $id = filter($this->post('id_media'));
        
		$data = array();
        if ($id == FALSE)
		{
			$data['id_media'] = 'required';
			$validation = "error";
			$code = 400;
		}
        ...
        if ($validation == "ok")
		{
            $query = $this->the_model->info(array('id_media' => $id));
	...
	function info_get()
	{
		...
		$id_media = filter($this->get('id_media'));
	...
	function lists_get()
	{
		...
		if (in_array($order, $this->config->item('default_media_order')) && ($order == TRUE))
		{
			$order = $order;
		}
		else
		...
		$param = array();
		$param2 = array();
		if ($id_media_album == TRUE)
		{
			$param['id_media_album'] = $id_media_album;
			$param2['id_media_album'] = $id_media_album;
		}
	...
	function update_post()
	{
		...
		$id_media = filter($this->post('id_media'));
		$id_media_album = filter($this->post('id_media_album'));
		$media_url = filter($this->post('media_url'));
		...
		if ($validation == 'ok')
		{
			$query = $this->the_model->info(array('id_media' => $id_media));
			...
			if ($query->num_rows() > 0)
			{
				...
				{
					$param['updated_date'] = date('Y-m-d H:i:s');
					$update = $this->the_model->update($id_media, $param);
