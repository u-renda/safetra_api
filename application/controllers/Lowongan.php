...
class Lowongan extends REST_Controller {
    ...
    {
        parent::__construct();
		$this->load->model('lowongan_model', 'the_model');
    }
	...
	function delete_post()
	{
        ...
        $id = filter($this->post('id_lowongan'));
        
		$data = array();
        if ($id == FALSE)
		{
			$data['id_lowongan'] = 'required';
			$validation = "error";
			$code = 400;
		}
        ...
        if ($validation == "ok")
		{
            $query = $this->the_model->info(array('id_lowongan' => $id));
	...
	function info_get()
	{
		...
		$id_lowongan = filter($this->get('id_lowongan'));
	...
	function update_post()
	{
		...
		$id_lowongan = filter($this->post('id_lowongan'));
		$name = filter(trim($this->post('name')));
		$description = filter(trim($this->post('description')));
		...
		if ($validation == 'ok')
		{
			$query = $this->the_model->info(array('id_lowongan' => $id_lowongan));
			...
			if ($query->num_rows() > 0)
			{
				...
				{
					$param['updated_date'] = date('Y-m-d H:i:s');
					$update = $this->the_model->update($id_lowongan, $param);
