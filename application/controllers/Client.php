...
class Client extends REST_Controller {
    ...
    {
        parent::__construct();
		$this->load->model('client_model', 'the_model');
    }
	...
	function create_post()
	{
		...
		$client_url = filter(trim($this->post('client_url')));
		...
		if (check_client_name($name) == FALSE && $name == TRUE)
		{
	...
	function delete_post()
	{
        ...
        $id = filter($this->post('id_client'));
        
		$data = array();
        if ($id == FALSE)
		{
			$data['id_client'] = 'required';
			$validation = "error";
			$code = 400;
		}
        ...
        if ($validation == "ok")
		{
            $query = $this->the_model->info(array('id_client' => $id));
	...
	function info_get()
	{
		...
		$id_client = filter($this->get('id_client'));
	...
	function lists_get()
	{
		...
		if (in_array($order, $this->config->item('default_client_order')) && ($order == TRUE))
		{
			$order = $order;
		}
		else
	...
	function update_post()
	{
		...
		$id_client = filter($this->post('id_client'));
		$name = filter(trim($this->post('name')));
		$logo = filter($this->post('logo'));
		$client_url = filter(trim($this->post('client_url')));
		...
		if ($validation == 'ok')
		{
			$query = $this->the_model->info(array('id_client' => $id_client));
			...
			if ($query->num_rows() > 0)
			{
				...
				{
					$param['updated_date'] = date('Y-m-d H:i:s');
					$update = $this->the_model->update($id_client, $param);
